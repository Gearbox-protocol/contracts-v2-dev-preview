...
import {MiningApproval} from "../libraries/Types.sol";
...
contract AccountFactory is IAccountFactory, ACLTrait, ReentrancyGuard {
    ...
    MiningApproval[] public miningApprovals;
    ...
    function addMiningApprovals(MiningApproval[] calldata _miningApprovals)
        external
        configuratorOnly // T:[AF-13]
    {
        require(!isMiningFinished, Errors.AF_MINING_IS_FINISHED); // T:[AF-17]
        for (uint256 i = 0; i < _miningApprovals.length; i++) {
            ...
            MiningApproval memory item = MiningApproval(
