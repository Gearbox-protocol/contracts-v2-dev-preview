...
contract PoolService is IPoolService, ACLTrait, ReentrancyGuard {
     ...
     *
     * #if_succeeds {:msg "After addLiquidity() the pool gets the correct amoung of underlying(s)"}
     *      IERC20(underlying).balanceOf(address(this)) == old(IERC20(underlying).balanceOf(address(this))) + amount;
        ...
        nonReentrant
    {
        ...
        uint256 balanceBefore = IERC20(underlying).balanceOf(
            address(this)
        );
     ...
     * #if_succeeds {:msg "After removeLiquidity() `to` gets the liquidity in underlying(s)"}
     *      (to != address(this) && to != treasuryAddress) ==>
     *          IERC20(underlying).balanceOf(to) == old(IERC20(underlying).balanceOf(to) + (let t:= fromDiesel(amount) in t.sub(t.percentMul(withdrawFee))));
     * #if_succeeds {:msg "After removeLiquidity() treasury gets the withdraw fee in underlying(s)"}
     *      (to != address(this) && to != treasuryAddress) ==>
     *          IERC20(underlying).balanceOf(treasuryAddress) == old(IERC20(underlying).balanceOf(treasuryAddress) + fromDiesel(amount).percentMul(withdrawFee));
        ...
        returns (uint256)
    {
        ...
        uint256 underlyingsAmount = fromDiesel(amount); // T:[PS-3, 8]

        uint256 amountTreasury = underlyingsAmount.percentMul(withdrawFee);
        uint256 amountSent = underlyingsAmount - amountTreasury;

        IERC20(underlying).safeTransfer(to, amountSent); // T:[PS-3, 34]
        ...
        _expectedLiquidityLU = _expectedLiquidityLU - underlyingsAmount; // T:[PS-3, 8]
        ...
        whenNotPaused // T:[PS-4]
    {
        ...
        IERC20(underlying).safeTransfer(creditAccount, borrowedAmount); // T:[PS-14]
